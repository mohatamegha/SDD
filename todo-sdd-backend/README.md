...
- Keep the OpenAPI spec in todo-sdd/specs as the single source of truth. Avoid duplicating the spec; instead configure the app or CI to serve/regenerate from that canonical file.
- Consider adding Spectral linting in CI to validate the OpenAPI spec automatically on PRs.
- The IDE flagged a transitive jackson-core advisory—consider upgrading Spring Boot or jackson if you need to address CVE concerns.
- I can add integration tests, Swagger UI enablement, or a generation workflow to keep server code in sync with the spec.
